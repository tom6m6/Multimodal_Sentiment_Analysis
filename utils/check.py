...
if __name__ == '__main__':
    ...
    parser.add_argument('--batchsize', type=int, default=1024, help='128 512 1024')
    parser.add_argument('--model_loc', type=str, default='saved/saved_models/clip.pth',help='saved models\' location')
    ...
    df["pred"] = res
    df["pred"] = df["pred"].map(int2cat)
    df["diff"] = df["tag"] != df["pred"]
    df.to_csv("output/tmp_no_open_check.txt", index=False)
    ...
    print(neg2pos, pos2neg, neu2pos, neu2neg, pos2neu, neg2neu, 1- cnt / len(feats_text))